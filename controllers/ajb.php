...
class ajb extends CI_Controller
{
    ...
    public function update() {
        ...
        if ($this->form_validation->run()) {
            ...
            //STPD
            $this->proses_stpd($this->input->post('id'));
    ...
    function proses_stpd($sspd_id) {
        //cek tgl ajb < tgl bayar
        $this->load->model('penerimaan_model');
        $bank = $this->penerimaan_model->get_by_sspd($sspd_id);

        $tgl_ajb = date('Y-m-d', strtotime($this->input->post('tgl_ajb')));
        $tgl_bayar = date('Y-m-d', strtotime($bank->tanggal));
        
        if($tgl_ajb < $tgl_bayar) {
            $this->sspd_model->proses_stpd($sspd_id);
        }
    }
