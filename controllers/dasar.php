...
class Dasar extends CI_Controller {
	...
	function __construct() {
        ...
        $this->load->model(array('dasar_model'));
	}
	...
	public function index() {
		...
		$this->load->view('vdasar', $data);
	}
	
	function grid() {
		$i=0;
		$responce = new stdClass();
		if($query = $this->dasar_model->get_all()) {
	...
	public function add() {
		if(!$this->module_auth->create) {
			...
			redirect(active_module_url('dasar'));
		}
		$data['current']     = 'referensi';
		$data['apps']    = $this->apps_model->get_active_only();
		$data['faction']     = active_module_url('dasar/add');
		$data['dt']          = $this->fpost();
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->dasar_model->save($data);
			
			$this->session->set_flashdata('msg_success', 'Data telah disimpan');		
			redirect(active_module_url('dasar'));
		}
		$this->load->view('vdasar_form',$data);
	}
	...
	public function edit() {
		...
		$data['current']   = 'referensi';
		$data['apps']    = $this->apps_model->get_active_only();
		$data['faction']   = active_module_url('dasar/update');
			
		$id = $this->uri->segment(4);
		if($id && $get = $this->dasar_model->get($id)) {
	...
	public function update() {
		...
		if ($this->form_validation->run() == TRUE) {	
			...
			$this->dasar_model->update($this->input->post('cur_id'), $data);
	...
	public function delete() {
		...
		$id = $this->uri->segment(4);
		if($id && $this->dasar_model->get($id)) {
			$this->dasar_model->delete($id);
