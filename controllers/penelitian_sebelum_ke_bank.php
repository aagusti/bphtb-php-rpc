...
class Penelitian extends CI_Controller
{
    ...
    function grid()
    {
        ...
        $this->datatables->select('coalesce(v.id, ss.id) as id, {rownum} as rownum,
        ...
        $this->datatables->join('bphtb_validasi v', 'v.sspd_id=ss.id', 'left');
                
        if($this->input->get('sSearch') == '')
            $this->datatables->filter("date(ss.tgl_transaksi) between '{$tgl1}' and '{$tgl2}'");
        
        $this->datatables->where("date(ss.tgl_transaksi) between '{$tgl1}' and '{$tgl2}'");
        
        if($proses_id == 1) {
            if(defined('BPHTB_NEED_APPROVAL') && BPHTB_NEED_APPROVAL==TRUE)
                $this->datatables->where("(v.id is null and ss.status_pembayaran>0 and sa.id is not null or (ss.bphtb_harus_dibayarkan = 0 and ss.status_validasi <> 1))");
            else
                $this->datatables->where("(v.id is null and ss.status_pembayaran>0 or (ss.bphtb_harus_dibayarkan = 0 and ss.status_validasi <> 1))");
            
        } else {
            $this->datatables->where("v.id is not null");
        }
    ...
    public function do_validasi()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $bank_id = ($bank) ? $this->penerimaan_model->get_by_sspd($sspd_id)->id : 0;
    ...
    public function edit()
    {
        ...
        if ($id && $this->sk_model->get_by_validasi_id($id)) {
