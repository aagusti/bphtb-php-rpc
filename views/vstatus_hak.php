...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
		...
		"bInfo": true,
		"bProcessing": false,
		"sAjaxSource": "<?=active_module_url();?>status_hak/grid"
	});
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>status_hak/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>status_hak/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>status_hak/delete/'+mID;
			};
		}else{
...
<div class="content">
    <div class="container-fluid">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#"><strong>Status Kepemilikan</strong></a>
			</li>
		</ul>
