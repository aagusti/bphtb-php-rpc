...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
		...
		"bInfo": true,
		"bProcessing": false,
		"sAjaxSource": "<?=active_module_url();?>alasan/grid"
	});
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>alasan/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>alasan/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>alasan/delete/'+mID;
			};
		}else{
...
<div class="content">
    <div class="container-fluid">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#"><strong>Alasan Pengurangan</strong></a>
			</li>
		</ul>
