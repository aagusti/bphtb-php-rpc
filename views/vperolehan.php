...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
		...
		"bInfo": true,
		"bProcessing": false,
		"sAjaxSource": "<?=active_module_url();?>perolehan/grid"
	});
	...
	$('#btn_tambah').click(function() {
		window.location = '<?=active_module_url();?>perolehan/add/';
	});

	$('#btn_edit').click(function() {
		if(mID) {
			window.location = '<?=active_module_url();?>perolehan/edit/'+mID;
		}else{
	...
	$('#btn_delete').click(function() {
		if(mID) {
			...
			if(hapus==true) {
				window.location = '<?=active_module_url();?>perolehan/delete/'+mID;
			};
		}else{
